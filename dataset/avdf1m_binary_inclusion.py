...
class AVDF1M(Dataset):
    ...
    def __init__(self, subset: str, root: str = "data", frame_padding: int = 512,
        ...
        if metadata is None:
            ...
            if self.subset == 'trainset':
                txt_path = os.path.join(self.root, 'trainset_label.txt')
            else:
                txt_path = os.path.join(self.root, 'valset_label.txt')
    ...
    def __getitem__(self, index: int) -> List[Tensor]:
        ...
        video, audio, _ = read_video(os.path.join(self.root, self.subset, meta['file']))
        ...
        if not self.require_match_scores:
            ...
            if meta['target'] == "1":
                video_level_label = 1
            # if meta['modify_type'] == "fake_audio_real_visual":
            #     video_level_label = 1
            # if meta['modify_type'] == "fake_audio_fake_visual":
            #     video_level_label = 1
            outputs = [video, audio] + [str(video_level_label)] #+ self.get_meta_attr(meta, video, audio, label)
        else:
...
class AVDF1MDataModule:
    def __init__(self, root: str = "data", frame_padding=512, max_duration=40,
                 ...
                 batch_size: int = 1, num_workers: int = 0,take_train: int = None, take_dev: int = None, take_test: int = None,
    ...
    def setup(self) -> None:
        self.metadata_train: List[Metadata] = read_json(os.path.join(self.root, "train_metadata.json"), lambda x: Metadata(**x))
        self.metadata_val: List[Metadata] = read_json(os.path.join(self.root, "val_metadata.json"), lambda x: Metadata(**x))
        self.metadata = self.metadata_train + self.metadata_val
    ...
    def create_data_loader(self, dataset, shuffle: bool = False, sampler = None) -> DataLoader:
        if sampler is None:
            return DataLoader(dataset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=shuffle)
        else:
            return DataLoader(dataset, batch_size=self.batch_size, num_workers=self.num_workers, sampler=sampler)

    def get_train_dataloader(self) -> DataLoader:
        sampler = RandomSampler(self.train_dataset, num_samples=self.take_train, replacement=True) if self.take_train is not None else None
        return self.create_data_loader(self.train_dataset, sampler=sampler)
...
if __name__ == '__main__':
    # metadata_train: List[Metadata] = read_json(os.path.join('/home/zoloz/8T-2/zhanyi/data/AV-Deepfake1M/unzip', "train_metadata.json"), lambda x: Metadata(**x))
    dataset = AVDF1M(
            "trainset",
            root='/home/zoloz/8T-2/zhanyi/data/openset_df_competition/multiFFDV/phase1',
            frame_padding=512,
