...
class LrLogger():
    ...
    @staticmethod
    def log_learning_rate(epoch, optimizer) -> None:
        for i, params_group in enumerate(optimizer.param_groups):
            key = f"opt_lr{i}"
            value = params_group["lr"]
...
def write_loss_to_file(loss, filepath='/home/zoloz/8T-2/zhanyi/code/LAV-DF/training.log'):
    # open to write loss
